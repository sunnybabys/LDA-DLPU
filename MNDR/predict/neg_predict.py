...
from data_input import Neg_DataLoader
from hyperparams import hyperparams as params


data = Neg_DataLoader("../data")
...
for i in range(params.number):
    ...
    for train_index, test_index in kf.split(data.dataset,dataset_label):
        ...
        feature_test =  data.dataset[test_index][data.dataset[test_index][:,-1] !=-1][:,:-1]
        target_train = dataset_label[train_index]
        target_test = dataset_label[test_index][dataset_label[test_index] != -1]
        print('begin training:')
        model = transNet(params.col_num, 120, 1).to(params.device)
        ...
        f = open("../data/data1_cv3_auc.csv", mode="a")
        ...
        f = open("../data/data1_cv3_aupr.csv", mode="a")
